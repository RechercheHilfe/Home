...
function loadTwitterWidgets(targetElement) {
    ...
    // Sicherheits-Timeout: Falls Twitter zu lange braucht oder blockiert wird, Spinner nach 20 Sek. entfernen
    ...
    const finishLoading = (success) => {
        ...
        if (isPostingsInitialRender && targetElement.id === 'contentArea') {
            console.warn(`%c[Twitter Embeds] Initial render detected. Triggering re-click.`, 'color: orange; font-weight: bold;');
            ...
            }, 500); // Kürzere Verzögerung für besseres UX
        } else {
            // Hier sicherstellen, dass der Spinner IMMER ausgeblendet wird
            setTimeout(() => {
                hideOverlayLoadingSpinner();
                console.log(`%c[Twitter Embeds] Embeds loaded, spinner hidden.`, 'color: #1DA1F2;');
            }, 500);
        }
    };
    ...
    if (window.twttr && window.twttr.widgets) {
        window.twttr.widgets.load(targetElement)
            .then(() => finishLoading(true))
            .catch(() => finishLoading(false));
    } else if (!twitterWidgetsLoaded) {
        // ... (Dein existierender Code zum Laden des Scripts ist okay, 
        // sollte aber auch finishLoading im onload/onerror aufrufen)
        ...
        script.onerror = () => finishLoading(false);
        document.body.appendChild(script);
        twitterWidgetsLoaded = true;
    } else {
        // Falls widgets.js bereits da ist, aber twttr noch nicht bereit war
        finishLoading(true);
    }
}
